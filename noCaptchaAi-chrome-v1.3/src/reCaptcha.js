(async () => {
  ...
  async function getBase64FromUrl(url) {
    ...
    if (!response.ok)
      return new Error(`Failed to fetch URL. Status code: ${response.status}`);
  ...
  const rcFast = (f) => {
    window.addEventListener("load", () => {
      ...
      s.insertRule(
        ...
        s.insertRule(
          `.rc-imageselect-tile {transition-duration: ${4 / f}s !important}`,
          1
        ),
        s.insertRule(
        ...
        s.insertRule(
          `.rc-image-tile-overlay {transition-duration: ${1 / f}s !important}`,
          4
        ),
        s.insertRule(
